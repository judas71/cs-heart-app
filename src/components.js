(function () {
  ...
  function FeesView({ athletes, fees, onSaveFee, onResetMonth }) {
    ...
    return h(
      ...
      h(
        ...
        h("button", { className: "danger align-end", onClick: () => onResetMonth(month, listedAthletes.map((athlete) => athlete.id)), disabled: !listedAthletes.length }, "Reset lună")
      ),
      h(
        "div",
  ...
  function ReportsView({ athletes, trainings, fees }) {
    ...
    const paidFees = fees.filter((fee) => fee.month === month && fee.status === "plătită" && athletesInFilter.some((athlete) => athlete.id === fee.athleteId));
    const totalPaid = paidFees.reduce((sum, fee) => sum + Number(fee.amountPaid || 0), 0);
    ...
    return h(
      ...
      h("div", { className: "report-grid" },
        ...
        h("div", { className: "report-block" }, h("h2", null, "Taxe plătite pe lună"), paidFees.length ? h("ul", { className: "clean-list" }, paidFees.map((fee) => { const athlete = athletes.find((item) => item.id === fee.athleteId); return h("li", { key: fee.id }, h("span", null, athlete ? athleteName(athlete) : "Sportiv șters"), h("strong", null, formatMoney(fee.amountPaid))); })) : h(EmptyState, { title: "Nu sunt taxe plătite.", text: "Schimbă luna sau marchează plăți în secțiunea Taxe." }))
      )
    );
  }
